...
Linear differential equations are ODE of the form $x' = a$. The exact solution for a _time horizon_ of $t$ is given as $x(t) = x(0) + a.t$. Affine differential equations are ODE of the form $x' = ax + b$. The exact solution for a _time horizon_ of $t$ is given as $x(t) = (x(0) + b/a).e^t - b/a$, where $e$ is the Euler number.
